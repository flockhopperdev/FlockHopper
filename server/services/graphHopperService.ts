...
import { createCircle, generateDirectionalCone } from '../utils/geo.js';
import {
...
const GRAPHHOPPER_ENDPOINT = process.env.GRAPHHOPPER_ENDPOINT || 'http://localhost:8989';

// Warn about localhost endpoint in production builds
if (process.env.NODE_ENV === 'production' && GRAPHHOPPER_ENDPOINT.includes('localhost')) {
  console.error('[GraphHopper] CRITICAL: Using localhost endpoint in production! Set GRAPHHOPPER_ENDPOINT to your production server.');
}
